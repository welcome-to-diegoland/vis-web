/**
 * Google Apps Script Proxy para vis-web - VERSIÓN CORREGIDA
...
function doGet(e) {
  try {
    ...
    // Crear respuesta simple (sin setHeaders que no funciona en Apps Script)
    const response = ContentService.createTextOutput(output).setMimeType(mimeType);
    return response;
...
/**
 * Función de prueba para asset_groups
 */
function testAssetGroups() {
  const mockEvent = {
    parameter: {
      sheet: 'asset_groups',
      format: 'csv'
  ...
  const result = doGet(mockEvent);
  console.log('🧪 Asset Groups test result:', result.getContent().substring(0, 200) + '...');
}

/**
 * Función para listar todas las pestañas disponibles
 */
function listSheets() {
  const SPREADSHEET_ID = '1TU51Xxx50DX5dc_aM9X2xguGBYV_Lsaswztv7WOmoyw';
  const spreadsheet = SpreadsheetApp.openById(SPREADSHEET_ID);
  ...
  sheets.forEach((sheet, index) => {
    console.log(`${index + 1}. ${sheet.getName()} (${sheet.getLastRow()} filas, ${sheet.getLastColumn()} columnas)`);
  });
}